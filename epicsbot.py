...
# deployment auth string 
# https://discord.com/oauth2/authorize?client_id=895831560052412417&permissions=380105005120&scope=bot
#
# Grab the API token from the .env file.
#DISCORD_TOKEN = os.getenv("DISCORD_TOKEN")
DISCORD_TOKEN="add token here or .env"
...
async def on_message(message):
	if message.content == "spreadsheet":
	        brief="shares the link of the working spreadsheet."
		await message.channel.send("https://docs.google.com/spreadsheets/d/1PUnULWr8UCf6FE13ih_Hhr60M6iwqR_RcmzRBREdCj8/edit#gid=399430277")
	if message.content == "concept":
	        brief="describes the project concept."
		await message.channel.send("Carbon trading is taking many forms now, and there are impacts in human and ecosystem rights which must be explored. This project aims to assess some of the large and small carbon and plastics brokering services currently being partonized by consumer and corporate customers.")
...
async def box(ctx, *args):
	...
	print("hello there : flag00 00")
	ELEMENT = ctx.message.content
	print("hello there : flag00 00 elem:" + ELEMENT)
	SUBELEMENT =  ELEMENT.split(' ')
	print("hello there : flag00 00")
	SUBSUB0 =  SUBELEMENT[1]  
	print("hello there : Flag: " + SUBSUB0)
	SUBSUB = SUBSUB0[0 : 1]
	if (SUBSUB == "-"):
		print("helllo there : flag00 2a")
		print("helllo there : flag00 2a" + SUBSUB)
		SUBSUB2 = 9936 + int(SUBSUB0)
		LE_num = SUBSUB0[1 : 3] 
		print("helllo there : flag00 2b")
	...
	print("hello there : flag  : ")
	# get token data from api.opensea.io
	NFTdata = requests.request("GET", API_URL)
	print("hello there : flag  :1 ")
	print ("1111")
	print("hello there : flag  :2 ")
